...
// .\zad-thr.exe 10 10 -5 15 macierz_wynikowa.csv 12

// .\zad-thr.exe mTest.csv macierz_wynikowa.csv 12
...
// Funkcja do sortowania wierszy macierzy w wątku
void sortMatrixRow(std::vector<double>& row) {
    std::sort(row.begin(), row.end());
}

// Funkcja do sortowania macierzy z wykorzystaniem wątków
void sortMatrixRows(std::vector<std::vector<double>>& matrix, int num_threads) {
    std::vector<std::thread> threads;
    int rows_per_thread = matrix.size() / num_threads;
    int remaining_rows = matrix.size() % num_threads;

    auto sortRows = [&](int start, int end) {
    ...
    for (int i = 0; i < num_threads; ++i) {
        ...
        threads.emplace_back(sortRows, start, end);
        start = end;
    }
